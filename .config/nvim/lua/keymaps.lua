...
map('n', '<C-q>', ':confirm qall<cr>', { silent = true })
...
map('n', '<cr>', function()
  -- In the quickfix/location list (and other special buffers), <CR> must keep
  -- its native behaviour (jump to the entry), so only toggle hlsearch in
  -- ordinary buffers.
  if vim.bo.buftype == '' and vim.v.hlsearch == 1 then
    vim.cmd('nohlsearch')
  else
